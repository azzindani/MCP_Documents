...
NAMES = ("read", "edit")
PUBLIC = "https://docs.example.test"
...
out = {}
with TestClient(unified_server.app, base_url=os.environ["DOCS_PUBLIC_URL"]) as c:
...
def discovery(tmp_path_factory) -> dict:
    ...
    env = {**os.environ, "DOCS_API_KEY": "test-key-not-a-secret", "DOCS_PUBLIC_URL": PUBLIC}
    for name in (*NAMES, "domain"):
        env[f"DOCS_{name.upper().replace('-', '_')}_OAUTH_STATE_DIR"] = str(state / name)
