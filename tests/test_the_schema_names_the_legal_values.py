...
def _dispatch_params():
    from servers.docs_edit import server as edit
    from servers.docs_read import server as read

    for app, label in [(read.mcp, "docs-read"), (edit.mcp, "docs-edit")]:
...
def test_the_census_finds_them():
    ...
    assert len(found) >= 3, f"only {len(found)} dispatch parameters seen -- did the census break?"
